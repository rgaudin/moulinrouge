...
def get_html_for( title )
	...
	puts "#{title}\n"	
	File.open( "/tmp/title", "w+" ) { |f| f.write "Kategorio:#{title}" }
...
#db = DBI.connect("DBI:Mysql:wikipedia", "root", "moulin")
begin
...
# TMP
TOTAL_ARTICLES = @dbh.select_one("SELECT COUNT(*) FROM page WHERE page_namespace=14")[0].to_i
...
DATA_PATH	= "/var/moulin/eo/encyclopedia-categories/"
INDEX_PATH	= "/var/moulin/eo/encyclopedia-categories"
...
res = @dbh.execute("SELECT page_title, page_is_redirect, page_latest FROM page WHERE page_namespace=14 ORDER BY page_title ASC LIMIT #{ROW_START_INDEX},#{NB_ARTFORME} ")
